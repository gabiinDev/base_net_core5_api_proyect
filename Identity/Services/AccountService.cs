...
namespace Identity.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<Response<AuthenticationResponse>> AuthenticateASync(AuthenticationRequest request, string ipAddress)
        {
            ...
            if (usuario == null)
                throw new ApiException($"No se encontró un usuario registrado con el correo {request.Email}.");

            var result = await _signInManager.PasswordSignInAsync(usuario.UserName, request.Password, false, lockoutOnFailure: false);
            if (!result.Succeeded)
                throw new ApiException($"Losdatos ingresados no son validos.");
            ...
            return new Response<AuthenticationResponse>(response, $"Usuario Autenticado");
        }
        ...
        public async Task<Response<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (usuarioConElMismoUserName != null)
                throw new ApiException($"El nombre de usuario {request.UserName} ya esta siendo utilizado.");
            ...
            else
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return new Response<string>(usuario.Id, message: $"Usuario registrado exitosamente.");
                }
                else
                {
        ...
        private async Task<JwtSecurityToken> GenerateJWToken(ApplicationUser usuario)
        {
